...
@section('page_title', 'Berita dan Artikel Pembina')
@section('page_subtitle', 'Kelola publikasi kegiatan binaan dengan tampilan daftar, filter, dan editor yang lebih fokus.')
...
@php
    ...
    $listUrl = $listUrl ?? route('coach.articles.index', request()->except(['page', 'tab']) + ['tab' => 'list']);
    $writeUrl = $writeUrl ?? route('coach.articles.index', request()->except(['page', 'tab']) + ['tab' => 'write']);
@endphp
        ...
        @if(! $isWriteTab)
            <div class="article-summary-grid">
                <article class="article-summary-card">
                    ...
                    <span class="hint">Seluruh artikel binaan Anda.</span>
                </article>
                <article class="article-summary-card">
                    <span class="label">Draft</span>
                    <span class="value">{{ $statistics['draft'] }}</span>
                    <span class="hint">Masih belum terlihat publik.</span>
                </article>
                <article class="article-summary-card">
                    <span class="label">Dijadwalkan</span>
                    <span class="value">{{ $statistics['scheduled'] }}</span>
                    <span class="hint">Akan tayang otomatis.</span>
                </article>
                <article class="article-summary-card">
            ...
            <section class="article-surface">
                <div class="article-list-card__head">
                    <div>
                        <h2>Daftar Artikel Binaan</h2>
                        <p>Fokuskan pencarian ke artikel yang Anda kelola, lalu saring berdasarkan kategori, kegiatan binaan, status, dan periode tayang.</p>
                    </div>
                </div>
                ...
                <div class="card-body pt-3">
                    <form method="get" class="article-filter-card">
                        ...
                        <button
                            ...
                            type="button"
                            data-article-filter-toggle="#coachArticleFilterPanel"
                            aria-expanded="{{ $hasActiveFilters ? 'true' : 'false' }}"
                        >
                            <i class="bi bi-funnel"></i>Filter Daftar Artikel
                        </button>

                        <div class="article-filter-collapse {{ $hasActiveFilters ? 'show' : '' }}" id="coachArticleFilterPanel">
                            <div class="article-filter-grid">
                                <div class="article-filter-field article-filter-search">
                                    <label for="coach_article_search">Cari judul</label>
                                    <input id="coach_article_search" type="text" name="search" value="{{ $search }}" class="form-control" placeholder="Cari judul artikel">
                                </div>

                                <div class="article-filter-field">
                                    <label for="coach_content_category">Kategori</label>
                                    <select id="coach_content_category" name="content_category" class="form-select">
                                ...
                                <div class="article-filter-field">
                                    <label for="coach_article_extracurricular_filter">Kegiatan</label>
                                    <select id="coach_article_extracurricular_filter" name="extracurricular_id" class="form-select">
                                ...
                                <div class="article-filter-field">
                                    <label for="coach_article_status_filter">Status</label>
                                    <select id="coach_article_status_filter" name="status" class="form-select">
                                ...
                                <div class="article-filter-field">
                                    <label for="coach_published_from">Tayang dari</label>
                                    <input id="coach_published_from" type="date" name="published_from" value="{{ $publishedFrom }}" class="form-control">
                                </div>

                                <div class="article-filter-field">
                                    <label for="coach_published_until">Sampai</label>
                                    <input id="coach_published_until" type="date" name="published_until" value="{{ $publishedUntil }}" class="form-control">
                                </div>
                                ...
                                <div class="article-filter-actions">
                                    ...
                                    <a href="{{ route('coach.articles.index') }}" class="btn btn-outline-secondary">
                            ...
                            <table class="table table-striped table-compact align-middle mb-0">
                                ...
                                @foreach($articles as $article)
                                    <tr>
                                        ...
                                        <td>{{ $article->extracurricular?->name ?? '-' }}</td>
                                        ...
                                        <td class="text-end">
                                            <div class="d-inline-flex align-items-center gap-2 flex-wrap justify-content-end">
                                                <a href="{{ route('coach.articles.edit', $article) }}" class="btn btn-sm btn-outline-primary"><i class="bi bi-eye"></i>Detail</a>
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end">
                                                        ...
                                                        @endif
                                                        <li><a class="dropdown-item" href="{{ route('coach.articles.preview', $article) }}" target="_blank"><i class="bi bi-display"></i>Pratinjau</a></li>
                                                        <li><a class="dropdown-item" href="{{ route('coach.articles.edit', $article) }}"><i class="bi bi-pencil-square"></i>Edit</a></li>
                                                        <li>
                                                            <form method="post" action="{{ route('coach.articles.duplicate', $article) }}">
                                                                @csrf
                                                        ...
                                                        @if($article->publication_status !== \App\Models\Article::STATUS_PUBLISHED)
                                                            <li>
                                                                <form method="post" action="{{ route('coach.articles.publish', $article) }}" class="article-confirmable-form" data-confirm-title="Publikasikan artikel?" data-confirm-message="Artikel ini akan langsung tampil pada halaman publik. Lanjutkan publikasi?" data-confirm-submit-label="Publikasikan" data-confirm-variant="primary">
                                                                    @csrf
                                                                    @method('patch')
                                                        ...
                                                        @if($article->publication_status === \App\Models\Article::STATUS_PUBLISHED)
                                                            <li>
                                                                <form method="post" action="{{ route('coach.articles.unpublish', $article) }}" class="article-confirmable-form" data-confirm-title="Tarik publikasi?" data-confirm-message="Artikel akan berhenti tampil pada halaman publik. Lanjutkan?" data-confirm-submit-label="Tarik Publikasi" data-confirm-variant="warning">
                                                                    @csrf
                                                                    @method('patch')
                                                        ...
                                                        @if($article->publication_status !== \App\Models\Article::STATUS_ARCHIVED)
                                                            <li>
                                                                <form method="post" action="{{ route('coach.articles.archive', $article) }}" class="article-confirmable-form" data-confirm-title="Arsipkan artikel?" data-confirm-message="Artikel akan dipindahkan ke arsip dan tidak tampil publik. Lanjutkan?" data-confirm-submit-label="Arsipkan" data-confirm-variant="warning">
                                                                    @csrf
                                                                    @method('patch')
                                                        ...
                                                        <li>
                                                            <form method="post" action="{{ route('coach.articles.destroy', $article) }}" class="article-confirmable-form" data-confirm-title="Hapus artikel?" data-confirm-message="Artikel ini akan dihapus permanen dan tidak bisa dikembalikan. Lanjutkan?" data-confirm-submit-label="Hapus Permanen" data-confirm-variant="danger">
                                                                @csrf
                                                                @method('delete')
                            ...
                            @foreach($articles as $article)
                                <div class="mobile-data-card">
                                    <div class="d-flex gap-3 align-items-start">
                                        ...
                                        <div class="flex-fill min-width-0">
                                            ...
                                            <div class="article-inline-meta">
                                                ...
                                                <span><i class="bi bi-diagram-3"></i>{{ $article->extracurricular?->name ?? '-' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mobile-data-card-actions">
                                        <a href="{{ route('coach.articles.edit', $article) }}" class="btn btn-outline-primary"><i class="bi bi-eye"></i>Detail</a>
                                        <div class="dropdown">
        ...
        @else
            <section class="article-surface article-surface--padded" id="article-create-panel">
                <div class="article-editor-shell__head">
                    <div>
                        <h2>Tulis Artikel</h2>
                        <p>Tulis artikel kegiatan binaan dengan susunan data, media, SEO, dan aksi publikasi yang lebih fokus.</p>
                    </div>
                ...
                <div class="card-body pt-3">
                    <form method="post" action="{{ route('coach.articles.store') }}" enctype="multipart/form-data">
                        @csrf
